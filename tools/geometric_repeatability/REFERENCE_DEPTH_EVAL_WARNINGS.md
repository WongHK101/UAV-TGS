...
- unstable training-only MVS reference geometry, especially facade holes,
  rounded/soft Poisson surfaces, and floating mesh fragments
...
After Building pilot inspection, the current fixed reference-construction
protocol is:

```text
PatchMatch max_image_size: 2000
PatchMatch source images: __auto__, 30
...
Reasoning from the Building pilot:

- `max_image_size=3000` did not visibly improve the mesh over `2000`, so the
  current protocol uses `2000` for better cost and reproducibility.
- `trim=7` left facade holes that were too large for reliable reference-depth
  evaluation, so the current protocol uses `trim=6`.
- `trim=5` and `trim=6` looked very similar, so `trim=6` was kept as the less
  aggressive option.
- `depth=10` reduced local bulging but lost much more building texture/detail.
- `depth=12` preserved visibly better roof/facade structure than `depth=10`.
- `point_weight=4` and `point_weight=6` looked nearly identical, so `6` was
  selected as the frozen default.
...
- hand-cropping only one scene's fused point cloud
- manually filling facade holes
- changing Poisson parameters scene by scene
...
Do not assume the reference mesh is reliable just because COLMAP produced a
mesh.
...
- low-support facade regions becoming soft or locally bulged in Poisson meshes
- Poisson meshes collapsing or becoming unusable when a small number of MVS
  outlier points expanded the global bounding box
...
- Soft Poisson fill can understate or overstate model depth error near facades.

Minimum checks:

- Inspect both `reference_fused_geometric.ply` and `reference_mesh_*.ply`.
- Open the fused point cloud and mesh side by side in CloudCompare or another
  3D viewer.
...
- insufficient MVS point support on vertical surfaces
- oblique views not providing stable matches for facade pixels
- overly strict PatchMatch or fusion consistency filtering

What we learned:

- Lowering Poisson `trim` from `7` to `6` reduced visible holes in Building.
- Lowering `trim` further to `5` did not produce a clearly better result.
- Increasing MVS resolution from `2000` to `3000` did not visibly improve
  Building, and the fused point count was nearly unchanged.

Practical fix in the frozen protocol:

- Use `trim=6`.
- Keep `max_image_size=2000`.
...
- Poisson output contains many low-confidence surface islands

Likely causes:

- MVS depth outliers survive fusion
- fusion thresholds are too permissive
- Poisson wraps sparse/noisy point support into surfaces

Potential fixes if this becomes a blocker:

- Make fusion stricter, for example by increasing `StereoFusion.min_num_pixels`
  or tightening reprojection/depth/normal consistency.
...
### 6.3 Poisson Mesh Collapse From Extreme Outliers

Symptoms:

- fused point cloud looks mostly plausible, but the Poisson mesh has only a tiny
...
- most Building fused points were near the scene, but a small fraction of
  outliers expanded the coordinate range to thousands of meters
- Poisson built its octree over this huge box, causing the actual building to be
  under-resolved

Potential fixes:

- Inspect robust quantiles and full coordinate min/max of the fused point cloud.
- Clip extreme coordinate outliers before Poisson meshing, using a fixed,
  documented rule such as robust quantile AABB plus padding.
...
- Poisson smoothing over missing data
- MVS depth noise near occlusion boundaries

What we learned:

- `depth=10` reduces bulging but removes too much visible structure and texture.
- `depth=12` preserves building structure better and is kept in the frozen
  protocol.
- `depth=13` is much heavier and more likely to preserve small fragments.
...
- outlier clipping
- mesh backend
- Poisson trim / depth / point weight
- density filtering
...
2. Inspect the fused point cloud manually.
...
- MVS fused point cloud outliers
- excessive reference masking
- stale metrics computed on an older reference version

## 10. Current Status Of Our Protocol

The earlier Building reference investigation produced several intermediate
meshes that should be treated as diagnostic only:

- initial meshes with severe facade holes
- meshes with complete overall outline but too many floating fragments
- Poisson collapse caused by extreme fused-point outliers
- `trim=7` meshes with unacceptable facade gaps
- `d10` meshes that reduced bulging but lost too much structure/detail
- `3K` MVS meshes that looked essentially the same as `2K` for Building

The current frozen protocol is the `2K / source30 / fusion3 / d12_trim6_pw6`
setting listed in Section 2. Metrics intended for paper-facing comparison
should be recomputed with this setting across all scenes and all methods.
